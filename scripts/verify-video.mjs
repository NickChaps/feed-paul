...
const metadata=await page.evaluate(()=>{const v=document.querySelector('video');return {duration:v.duration,width:v.videoWidth,height:v.videoHeight}});assert.equal(metadata.width,1080);assert.equal(metadata.height,1080);assert.ok(Math.abs(metadata.duration-68)<.1);
const frames=[];for(const t of [1,13,27,40,55,66.5]){
...
await page.evaluate(()=>{const v=document.querySelector('video');v.currentTime=0;return v.play()});await page.waitForFunction(()=>document.querySelector('video').ended,{},{timeout:80000});
