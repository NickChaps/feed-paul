...
const metadata=await page.evaluate(()=>{const v=document.querySelector('video');return {duration:v.duration,width:v.videoWidth,height:v.videoHeight}});assert.equal(metadata.width,980);assert.equal(metadata.height,780);assert.ok(Math.abs(metadata.duration-72.6)<.1);
const frames=[];for(const t of [1,3,15,30,50,70]){
...
await page.evaluate(()=>{const v=document.querySelector('video');v.currentTime=0;return v.play()});await page.waitForFunction(()=>document.querySelector('video').ended,{},{timeout:85000});
